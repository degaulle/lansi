...
def index():
    ...
    essay_text = db.execute("SELECT * FROM essays WHERE id=:user_id", user_id=session.get("user_id"))
    
    # number of user's essay entries
    essay_number = len(essay_text)
    
    ############## checks if feedback is available for any essays ##############
    ...
    return render_template("index.html", essay_number=essay_number, essay_text=essay_text)
...
def messages():
    ...
    # number of user's essay entries
        user_id=session.get("user_id"), essay_id=request.form.get("essay_id_value"), message=request.form.get("essay_text"), initials = session.get(""))
